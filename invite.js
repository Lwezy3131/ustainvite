...
client.on("guildMemberAdd", (member) => {
    ...
    guild.fetchInvites().then(invites => {        var invite = invites.find(_i => gi.has(_i.code) && gi.get(_i.code).uses < _i.uses) || gi.find(_i => !invites.has(_i.code)) || guild.vanityURLCode;
        ...
        if(channel){
            channel.send((`:inbox_tray: **${member.user}** Sunucumuza Hoşgeldin Seni davet eden **${invite.inviter}** kişisininin \`${total + bonus}\` daveti oldu.`)                 
	  ) } 
	}).catch(); 
});
...
client.on("guildMemberRemove", (member) => {
    ...
    db.add(`invites.${data.inviter}.leave`, 1);
     if(channel){
        ...
        channel.send((`:outbox_tray: **${member.user}** adlı kullanıcı sunucumuzdan ayrıldı. Kullanıcıyı davet eden **${user}** kişisinin **${Number(total) + Number(bonus)}** daveti kaldı! `))
     }
});
